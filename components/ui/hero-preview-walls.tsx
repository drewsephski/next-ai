  ...
  return (
    ...
    >
      {cards.map((card, index) => (
        <motion.div
          key={card.id}
          className="absolute bg-white dark:bg-neutral-900 rounded-2xl p-5 sm:p-6 md:p-8 shadow-xl border border-neutral-200 dark:border-white/8 flex flex-col justify-between text-left overflow-hidden"
          style={{
...
export function HeroPreviewWalls() {
  const { user } = useAuth();
  const getStartedHref = user ? "/dashboard" : "/sign-in";
  const CARDS = [
    {
      id: 0,
    ...
    <section className="relative w-full overflow-hidden bg-white dark:bg-black text-black dark:text-white py-16 sm:py-20 md:py-24">
      <div className="max-w-5xl mx-auto text-left px-4 sm:px-6">
        ...
        <h1 className="text-3xl sm:text-5xl md:text-6xl font-bold leading-tight mb-4">
          We build technology that <br/>moves your vision forward
        </h1>
        ...
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 mb-10 sm:mb-20">
          <Button asChild className="rounded-full px-6 py-3 text-sm font-medium">
            <Link href="/dashboard">
              Explore
            </Link>
          </Button>
          <Button asChild variant="outline" className="group rounded-full px-6 py-3 text-sm font-medium text-black dark:text-white hover:text-black dark:hover:text-black">
